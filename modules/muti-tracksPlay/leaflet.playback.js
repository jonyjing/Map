// UMD initialization to work with CommonJS, AMD and basic browser script
// include
(function(factory) {
	var L;
	...
	L.Playback.Util = L.Class.extend({
		statics : {
			...
			TimeStr : function(time) {
				...
				var tms = time / 1000;
				// var dec = (tms -
				// Math.floor(tms)).toFixed(2).slice(1);
				var mer = 'AM';
				if (h > 11) {
					h %= 12;
					mer = 'PM';
	...
	L.Playback.MoveableMarker = L.Marker.extend({
		initialize : function(startLatLng, options, feature) {
			...
			if (marker_options.labels) {
				if (this.bindLabel) {
					this.bindLabel(this.getPopupContent(), {
						noHide : true
					});
				} else if (this.bindTooltip) {
		...
		move : function(latLng, transitionTime) {
			...
			if (this._popup) {
				this._popup.setContent(this.getPopupContent() + "坐标:" + (this._latlng.lng).toFixed(6) + ","
						+ (this._latlng.lat).toFixed(6));
			}
			if (this.label) {
				this.label.setContent(this.getPopupContent() + "坐标:" + (this._latlng.lng).toFixed(6) + ","
						+ (this._latlng.lat).toFixed(6));
			}
			if (this._tooltip) {
				this.setTooltipContent(this.getPopupContent() + "坐标:" + (this._latlng.lng).toFixed(6) + ","
						+ (this._latlng.lat).toFixed(6));
			}
		},
	...
	L.Playback.Track = L.Class.extend({
		...
		initialize : function(geoJSON, options) {
			...
			var t = currSampleTime; // t is used to iterate
			// through tick times
		...
		setMarker : function(timestamp, options) {
			...
			if (lngLat) {
				...
				// hide the marker if its not present yet and
				// fadeMarkersWhenStale is true
	...
	L.Playback.TracksLineLayer = L.Class.extend({
		options : {
			colors : [ '#ff7f50', '#87cefa', '#da70d6', '#32cd32', '#6495ed', '#ff69b4', '#ba55d3', '#cd5c5c',
					'#ffa500', '#40e0d0', '#1e90ff', '#ff6347', '#7b68ee', '#00fa9a', '#ffd700', '#6b8e23', '#ff00ff',
		...
		initialize : function(map, options) {
			this._map = map;
			L.setOptions(this, options);
			this.tracksLineLayer = L.featureGroup([]);
			map.addLayer(this.tracksLineLayer);
		},
		clearLayer : function() {
			this.tracksLineLayer.clearLayers();
		},
		addLayer : function(geoJSON) {
			if (!L.Util.isArray(geoJSON)) {
				geoJSON = [ geoJSON ];
			}
			for (var i = 0; i < geoJSON.length; i++) {
				var lineLatlngs = L.GeoJSON.coordsToLatLngs(geoJSON[i].geometry.coordinates);
				var trackLine = L.polyline(lineLatlngs, {
				...
				if (typeof L.polylineDecorator && this.options.AddArrow) {
					trackLineArrow = L.polylineDecorator(trackLine, {
						patterns : [ {
							offset : 25,
							repeat : 50,
							symbol : L.Symbol.arrowHead({
								pixelSize : this.options.arrowSize||8,
								pathOptions : {
									fillOpacity : 1,
									weight : 1,
									stroke:true,
									color:'#d8d8d8',
									fillColor:this.options.arrowColor||'#fff'
								}
							})
						} ]
					});
					this.tracksLineLayer.addLayer(trackLineArrow);
				}
				if (geoJSON[i].properties.Label) {
					trackLine.bindLabel(geoJSON[i].properties.Label, {
						noHide : true
					});
				}
				var start_point = geoJSON[i].geometry.coordinates[0];
				var end_point = geoJSON[i].geometry.coordinates[geoJSON[i].geometry.coordinates.length - 1];
				this.tracksLineLayer.addLayer(trackLine);
				this.tracksLineLayer.addLayer(this._createMarker(L.GeoJSON.coordsToLatLng(start_point), "start"));
				this.tracksLineLayer.addLayer(this._createMarker(L.GeoJSON.coordsToLatLng(end_point), "end"));
			}
			this._map.addLayer(this.tracksLineLayer);
			this._map.fitBounds(this.tracksLineLayer.getBounds(), {
				maxZoom : 17
			});
		},
		_createMarker : function(markerLatlng, type) {
			if (type === "start") {
				var startIcon = L.divIcon({
					iconSize : new L.Point(30, 32),
					className : 'qi-div-icon',
					html : '',
					iconAnchor : new L.Point(15, 32)
				});
				var startMarker = L.marker(markerLatlng, {
					icon : startIcon
				});
				return startMarker;
			} else {
				var endIcon = L.divIcon({
					iconSize : new L.Point(30, 32),
					className : 'zhong-div-icon',
					html : '',
					iconAnchor : new L.Point(15, 32)
				});
				var endMarker = L.marker(markerLatlng, {
					icon : endIcon
				});
				return endMarker;
			}
	...
	L.Playback.PlayControl = L.Control.extend({
		...
		onAdd : function(map) {
			...
			L.DomEvent.on(this._button, 'click', stop).on(this._button, 'mousedown', stop).on(this._button, 'dblclick',
					stop).on(this._button, 'click', L.DomEvent.preventDefault).on(this._button, 'click', play, this);
	...
	L.Playback.SliderControl = L.Control.extend({
		...
		onAdd : function(map) {
			...
			L.DomEvent.on(this._slider, 'click', stop).on(this._slider, 'mousedown', stop).on(this._slider, 'dblclick',
					stop).on(this._slider, 'click', L.DomEvent.preventDefault)
			// .on(this._slider, 'mousemove',
			// L.DomEvent.preventDefault)
			.on(this._slider, 'change', onSliderChange, this).on(this._slider, 'mousemove', onSliderChange, this);
	...
	L.Playback = L.Playback.Clock.extend({
		statics : {
			...
			TracksLineLayer : L.Playback.TracksLineLayer,
		...
		initialize : function(map, geoJSON, callback, options) {
			...
			if (this.options.tracksLineLayer) {
				this._tracksLineLayer = new L.Playback.TracksLineLayer(map, options);
			}
		...
		clearData : function() {
			...
			if (this._tracksLineLayer) {
				this._tracksLineLayer.clearLayer();
			}
		},
		...
		addData : function(geoJSON, ms) {
			...
			if (this.options.tracksLineLayer) {
				this._tracksLineLayer.addLayer(geoJSON);
			}
		},
