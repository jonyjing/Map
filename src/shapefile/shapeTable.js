...
/* WEBPACK VAR INJECTION */(function(Buffer) {MapShaper.ShapefileTable=ShapefileTable;
var utils = MapShaper.utils;
function ShapefileTable(buf, encoding) {
	var reader = new DbfReader(buf, encoding), altered = false, table;
...
function DbfReader(src, encodingArg) {
	if (utils.isString(src)) {
		MapShape.stop("[DbfReader] Expected a buffer, not a string");
	}
	...
	function readHeader(bin) {
		...
		// Detect header terminator (LF is standard, CR has been seen in the
		// wild)
		...
		if (colOffs != header.recordSize) {
			MapShaper.error("Record length mismatch; header:", header.recordSize, "detected:", colOffs);
		}
		if (bin.peek() != 0x0D) {
			console
					.log('[dbf] Found a non-standard header terminator (' + bin.peek()
	...
	function getEncoding() {
		if (!encoding) {
			...
			if (!encoding) {
				// fall back to utf8 if detection fails (so GUI can continue
				// without further errors)
				encoding = 'utf8';
				MapShaper.stop("Unable to auto-detect the text encoding of the DBF file.\n" + Dbf.ENCODING_PROMPT);
			}
		}
		return encoding;
	}
	...
	// (Much faster in v8 and other engines than assigning a series of
	// properties
	// to an object)
	...
	function getRecordReader(fields) {
		var readers = fields.map(getFieldReader), eofOffs = findEofPos(bin), create = getRecordConstructor(), values = [];

		return function readRow(r) {
			var offs = getRowOffset(r), fieldOffs, field;
			for (var c = 0, cols = fields.length; c < cols; c++) {
				field = fields[c];
				fieldOffs = offs + field.columnOffset;
				if (fieldOffs + field.size > eofOffs) {
					MapShaper.stop('[dbf] Invalid DBF file: encountered end-of-file while reading data');
				}
	...
	function getFieldReader(f) {
		...
		if (type == 'I') {
			...
			console.log("[dbf] Field \"" + field.name + "\" has an unsupported type (" + field.type
	...
	function findStringEncoding() {
		var ldid = header.ldid, codepage = Dbf.lookupCodePage(ldid), samples = getNonAsciiSamples(50), only7bit = samples.length === 0, encoding, msg;

		// First, check the ldid (language driver id) (an obsolete way to
		// specify which
		// codepage to use for text encoding.)
		// ArcGIS up to v.10.1 sets ldid and encoding based on the 'locale' of
		// the
		// user's Windows system :P
		//
		if (codepage && ldid != 87) {
			// if 8-bit data is found and codepage is detected, use the
			// codepage,
			// except ldid 87, which some GIS software uses regardless of
			// encoding.
			encoding = codepage;
		} else if (only7bit) {
		...
		if (encoding && samples.length > 0) {
			...
			console.log("Sample text containing non-ascii characters:" + (msg.length > 60 ? '\n' : '') + msg);
		}
		return encoding;
	}
...
function DataTable(obj) {
	...
	if (utils.isArray(obj)) {
		...
		if (utils.isInteger(obj)) {
			...
			MapShaper.error("[DataTable] Invalid constructor argument:", obj);
		}
	}
...
var dataTableProto = {
	...
	addField : function(name, init) {
		...
		if (!utils.isNumber(init) && !utils.isString(init) && !useFunction) {
			MapShaper.error("DataTable#addField() requires a string, number or function for initialization");
		}
		if (this.fieldExists(name))
			MapShaper.error("DataTable#addField() tried to add a field that already exists:", name);
		if (!dataFieldRxp.test(name))
			MapShaper.error("DataTable#addField() invalid field name:", name);
	...
	getFields : function() {
		var records = this.getRecords(), first = records[0];
...
Dbf.languageIds = [ 0x01, '437', 0x02, '850', 0x03, '1252', 0x08, '865', 0x09, '437', 0x0A, '850', 0x0B, '437', 0x0D,
		'437', 0x0E, '850', 0x0F, '437', 0x10, '850', 0x11, '437', 0x12, '850', 0x13, '932', 0x14, '850', 0x15, '437',
		0x16, '850', 0x17, '865', 0x18, '437', 0x19, '437', 0x1A, '850', 0x1B, '437', 0x1C, '863', 0x1D, '850', 0x1F,
		'852', 0x22, '852', 0x23, '852', 0x24, '860', 0x25, '850', 0x26, '866', 0x37, '850', 0x40, '852', 0x4D, '936',
		0x4E, '949', 0x4F, '950', 0x50, '874', 0x57, '1252', 0x58, '1252', 0x59, '1252', 0x64, '852', 0x65, '866',
		0x66, '865', 0x67, '861', 0x6A, '737', 0x6B, '857', 0x6C, '863', 0x78, '950', 0x79, '949', 0x7A, '936', 0x7B,
		'932', 0x7C, '874', 0x86, '737', 0x87, '852', 0x88, '857', 0xC8, '1250', 0xC9, '1251', 0xCA, '1254', 0xCB,
		'1253', 0xCC, '1257' ];
...
Dbf.readStringBytes = function(bin, size, buf) {
	...
	for (var i = 0; i < size; i++) {
		c = bin.readUint8();
		if (c === 0)
			break; // C string-terminator (observed in-the-wild)
		if (count > 0 || c != 32) { // ignore leading spaces (e.g. DBF numbers)
			buf[count++] = c;
		}
	}
	// ignore trailing spaces (DBF string fields are typically r-padded w/
	// spaces)
...
Dbf.getAsciiStringReader = function() {
	...
	return function readAsciiString(bin, size) {
		var str = '', n = Dbf.readStringBytes(bin, size, buf);
		for (var i = 0; i < n; i++) {
...
Dbf.getEncodedStringReader = function(encoding) {
	var buf = new Buffer(256), isUtf8 = MapShaper.standardizeEncodingName(encoding) == 'utf8';
	return function readEncodedString(bin, size) {
		var i = Dbf.readStringBytes(bin, size, buf), str;
		if (i === 0) {
			str = '';
		} else if (isUtf8) {
			str = buf.toString('utf8', 0, i);
		} else {
			str = MapShaper.decodeString(buf.slice(0, i), encoding); // slice
																		// references
																		// same
																		// memory
		}
...
Dbf.getStringReader = function(encoding) {
	if (!encoding || encoding === 'ascii') {
		...
		// TODO: user browserify or other means of decoding string data in the
		// browser
		MapShaper.error("[Dbf.getStringReader()] Non-ascii encodings only supported in Node.");
	}
};
...
Dbf.readBool = function(bin, size) {
	var c = bin.readCString(size), val = null;
	if (/[ty]/i.test(c))
		val = true;
	else if (/[fn]/i.test(c))
		val = false;
...
Dbf.readDate = function(bin, size) {
	var str = bin.readCString(size), yr = str.substr(0, 4), mo = str.substr(4, 2), day = str.substr(6, 2);
