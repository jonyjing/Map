            ...
            for (var o in i.layers) {
                ...
                for (var c in a.features) {
                    ...
                    if (f instanceof Function && (f = f(l.properties, t.z)), f instanceof Array || (f = [f]), f.length) {
                    	var showZoom=f[0].showZoom|| this._map.minZoom;
                		if(this._map.getZoom()<showZoom){
                			break;
                		}
    ...
    _getVectorTilePromise: function (t) {
        ...
        return fetch(i).then(function (t) {
            ...
            for (var e in t.layers) {
            	if(self.options.vectorTileLayerStyles[e]){
            		var showZoom=self.options.vectorTileLayerStyles[e].showZoom|| self._map.minZoom;
            		if(self._map.getZoom()<self.options.vectorTileLayerStyles[e].showZoom){
            			continue;
            		}
            	}
...
L.VectorGrid.Slicer = L.VectorGrid.extend({
    ...
    _onClick: function (t) {
        var e, r = this._map.mouseEventToLayerPoint(t).subtract(this.getOffset()),
            i = [];
        for (var n in this._layers) e = this._layers[n], e.options.interactive && e._containsPoint(r) && !this._map._draggableMoved(e) && (L.DomEvent.fakeStop(t), i.push(e));
