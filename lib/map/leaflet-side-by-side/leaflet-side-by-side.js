...
L.Control.SideBySide = L.Control.extend({
  ...
  addTo: function (map) {
    ...
    this._addEvents()
    this.setLeftLayers(this._leftLayer).setRightLayers(this._rightLayer);
    this._updateLayers();
    return this
  },
  ...
  onRemove: function () {
    ...
    var leftLayer=this._leftLayer.layer;
    var rightLayer=this._rightLayer.layer;
    if (leftLayer) {
    	if(leftLayer instanceof L.LayerGroup || leftLayer instanceof L.FeatureGroup){
    		leftLayer.eachLayer(function(layer) {
    ...
    if (rightLayer) {
    	if(rightLayer instanceof L.LayerGroup || rightLayer instanceof L.FeatureGroup){
    		rightLayer.eachLayer(function(layer) {
                layer.getContainer().style.clip = "";
            })
    	}else{
    ...
    map.baseMapSwitchControl.setCurrentBaseLayers(leftLayer);
    this._removeEvents();
  ...
  setLeftLayers: function (leftLayer) {
	this._updateLeftLayer(leftLayer);
	return this;
  },

  setRightLayers: function (rightLayer) {
	this._updateRightLayer(rightLayer);
	return this;
  },
  ...
  _removeLeftLayer:function(){
	var layer=this._leftLayer.layer;
	map.removeLayer(layer);
  },
  
  _updateLeftLayer:function(leftLayer){
	  var id=leftLayer.id
	  var layer=leftLayer.layer;
	  if(this.leftLayerId==id){
		  return;
	  }
	  this._removeLeftLayer();
	  this._map.addLayer(layer);
	  this._leftLayer=leftLayer;
	  this.leftLayerId=id;
  },
  
  _removeRightLayer:function(){
	var layer=this._rightLayer.layer;
	map.removeLayer(layer);
  },
  
  _updateRightLayer:function(rightLayer){
	  var layer=rightLayer.layer;
	  var id=rightLayer.id;
	  if(this.rightLayerId==id){
		  return;
	  }
	  this._removeRightLayer();
	  this._map.addLayer(layer);
	  this._rightLayer=rightLayer;
	  this.rightLayerId=id;
  },
